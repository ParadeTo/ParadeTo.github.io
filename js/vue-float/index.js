...
new Vue({
  ...
  computed: {
    ...
    decimalsSteps: function () {
      ...
      return this.decimalsStepsRedundant.slice(0, len)
    },
    ...
    originE: function () {
      ...
      else return -(this.decimalsSteps.map(step => String(step.bit)).indexOf('1') + 1)
    },
    e: function () {
      ...
      let eStr = this.computeInteger(e).map(step => step.remainder).reverse().join('')
    ...
    integerStr: function() {
      return this.integerSteps.slice().reverse().map(step => step.remainder).join('')
    },
    decimalsStr: function() {
      return this.decimalsSteps.slice().map(step => step.bit).join('')
    },
    f: function () {
      if (String(this.number) === '0') {
        let str = ''
      ...
      if(integerLen > 0) {
        let str = (this.integerStr.substr(1) + this.decimalsStr).slice(0, this.factorComputed.f)
